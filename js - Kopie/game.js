...
console.log(this.obstacles2.length)
clear()
this.background.draw()
this.player.draw()
// here we add obstacle objects to the array every x frames
...
if (frameCount % 98 === 0) {
	// console.log('add coin')
	this.obstacles2.push(new Obstacle2(this.coinImage2))
...
this.obstacles2 = this.obstacles2.filter((obstacle2) => {
	// for 'this' to have the right context here (being the Game object)
	// we have to use an arrow function for the callback
	// console.log(this)
	if (obstacle2.collision(this.player) || obstacle2.x < 0) {
		return false
	} else {
		return true
	}
